...
独立部署包位于 `dirty-talk/`，已随仓库提供。整体上传该目录即可，详见 [部署说明](dirty-talk/README.md)。根目录保留开发源码与测试，部署包不含测试依赖。源码更新后需同步部署包，避免两个目录版本不一致。

默认监听 `0.0.0.0:4317`，端口可用 `PORT` 修改。为保留 Host 与跨站请求防护，服务器部署必须设置浏览器实际访问的 `PUBLIC_ORIGIN`；未设置时只接受 localhost / 127.0.0.1 的访问。

Linux 临时验证（替换为实际服务器 IP；仅测试示例，不要通过明文 HTTP 填写密钥）：

```sh
PUBLIC_ORIGIN=http://你的服务器IP:4317 npm start
```

正式部署应在 HTTPS 反向代理后运行：

```sh
PUBLIC_ORIGIN=https://talk.example.com npm start
```

代理转发到 `http://127.0.0.1:4317`，并保留浏览器原始 `Host`（含非默认端口）。服务不信任客户端提供的 `X-Forwarded-Host`。HTTPS 证书需由代理配置；设置 PUBLIC_ORIGIN 不会自动启用 TLS。防火墙应限制 4317 仅供代理或可信网络访问。

当前应用没有用户登录和限流；公开使用前需要在代理层设置访问控制与限流，避免接口被滥用。HTTP 公网访问也可能导致剪贴板功能不可用。本文没有执行服务器部署或修改防火墙。
...
服务端监听 `0.0.0.0`，通过 `PUBLIC_ORIGIN` 限制允许访问的来源，以白名单提供静态资源；校验请求来源、JSON 和输入长度；上游 HTTPS 请求固定解析到公开 IPv4 地址、禁止重定向；密钥不会写入磁盘或日志。页面变化会取消请求并通知上游停止，供应商已经完成的计算仍可能计费。上游 HTTPS 仅支持公开 IPv4 域名，不支持局域网模型、IPv6-only 服务、非 443 端口或原生 Anthropic/Gemini 专用协议。自定义入口需要 Chat Completions 兼容接口。
