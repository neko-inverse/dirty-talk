...
export function createApp({ modelCall = callModel, publicOrigin = process.env.PUBLIC_ORIGIN } = {}) {
  let configuredOrigin;
  if (publicOrigin) {
    const url = new URL(publicOrigin);
    if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password || url.search || url.hash || url.pathname !== '/') throw new Error('PUBLIC_ORIGIN must be an HTTP(S) origin without credentials, path or query.');
  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (!host || (configuredOrigin ? host !== configuredOrigin.host : !localHost)) throw new AppError('不允许的访问地址，请检查 PUBLIC_ORIGIN 配置。', 403);
      ...
      if (req.method === 'POST' && ['/api/rewrite', '/api/test'].includes(path)) {
        ...
        if (req.headers.origin && req.headers.origin !== (configuredOrigin?.origin || `http://${host}`)) throw new AppError('不允许跨站请求。', 403);
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  server.listen(port, '0.0.0.0', () => console.log(`Dirty Talk listening on 0.0.0.0:${port}; visit ${process.env.PUBLIC_ORIGIN || `http://127.0.0.1:${port}`}`));
}
